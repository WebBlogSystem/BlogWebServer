<template>
  <div class="adminLeft">
    ...
    <div>
      <Menu :open-names="['1','2', '3','4','5']" class="customuser" :active-name="leftCurrent">
        <Submenu name="4">
          <template slot="title">
            <Icon type="ios-paper" />
            用户管理
          </template>
          <MenuItem name="4-1" @click.native="goOtherPage('4-1')">管理用户</MenuItem>
        </Submenu>
        <Submenu name="5">
          <template slot="title">
            <Icon type="ios-paper" />
          ...
          <MenuItem name="5-1" @click.native="goOtherPage('5-1')">管理文章</MenuItem>
        </Submenu>
        <Submenu name="1">
          <template slot="title">
            <Icon type="ios-paper" />
  ...
  methods: {
    goOtherPage (pageIndex) {
      if (pageIndex !== this.leftCurrent) {
        ...
        } else if (pageIndex === "4-1") {
          this.$router.push("/home/usermanage")
        } else if (pageIndex === "5-1") {
          this.$router.push("/home/essaymanage")
        }
      }
    },
    initWebSocket () {
